...
import { MagnifyingGlassIcon, MagnifyingGlassPlusIcon, UserGroupIcon, UserIcon } from "@heroicons/react/24/solid";
...
const Header = ({ searchTitle, onChange, onClick }) => {
  return (
    <div className="fixed flex z-10 top-0 bg-gradient-to-bl from-teal-500 to-purple-900 dark:from-slate-900 dark:to-slate-950 dark:text-slate-200 text-white h-[70px] w-full">
      <div className="flex justify-center items-center my-auto w-1/3 md:w-[45%] space-x-2 order-1">
        <UserGroupIcon className="h-7 w-7"/>
        <h1 className="hidden md:flex font-bold text-lg">Address Book</h1>
      </div>
      <div className="flex justify-center items-center w-1/3 md:w-[10%] order-3 md:order-2">
        <ThemeButton />
      </div>
      <div className="flex justify-center items-center w-1/3 md:w-[45%] order-2 md:order-2">
